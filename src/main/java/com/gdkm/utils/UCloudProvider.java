...
public class UCloudProvider {
    ...
    public String upload(InputStream fileStream, String mimeType, String fileName,String head) {
        ...
        if (filePaths.length > 1) {
            if (head!=null) {
                generatedFileName = head + "/" + UUID.randomUUID().toString() + "." + filePaths[filePaths.length - 1];
            }else {
...
//        os = new FileOutputStream(new File(localDir, saveName));
        ...
        try {
            ...
            res.setHeader("Content-disposition","attachment;filename=\"" + URLEncoder.encode("theName.jpg", "UTF-8")+"\"");
